...
GCE (M1/M3)  ──→  GCS バケット  ──→  Windows PC (M4)
  cron ジョブ      gcloud rsync       pull_from_gcs.ps1
```

M1・M3 は cron で実行され、結果（動画・画像・JSON・SQLite DB）を GCS バケットにプッシュします。
Windows の放送局は配信前に `pull_from_gcs.ps1` でデータを取得します。
...
├── pull_from_gcs.ps1          # Windows: GCS データをローカルに取得
...
**GCE → GCS** は `gcp/` 内のスクリプトで処理します（cron または `gcloud storage rsync` で手動実行）。
...
- [ ] **GCE→GCS 同期スクリプト**: `gcp/` ディレクトリで同期を参照しているが、専用のプッシュスクリプトがコミットされていない。cron ベースの `gcloud storage rsync` コマンドをドキュメント化または追加する。
- [ ] **CI/CD**: 自動テストおよびリンターが未設定。
- [ ] **content_index.py ドキュメント**: M1/M3（プロデューサー）と M4（コンシューマー）の間で使用される共有コンテンツレジストリのスキーマをドキュメント化する。
