...
def _fit_to_1920x1080(img: Image.Image) -> Image.Image:
    """画像を 1920x1080 にフィットさせる（レターボックス/ピラーボックス）"""
...
def generate_title_slide(
    ...
    try:
        ...
        if not _save_image_from_response(response, output_path):
            logger.warning("タイトル画像のレスポンスに画像パーツがありません。プレースホルダーを使用します。")
            _make_placeholder_image("Title Slide", output_path)
    except Exception as e:
        logger.error(f"タイトルスライド生成失敗: {e}")
        _make_placeholder_image("Title Slide", output_path)
...
def generate_content_images(
    ...
    logger.info(f"原稿中に{len(markers)}個の画像マーカーを検出しました。")
    ...
    for i, description in enumerate(markers):
        ...
        logger.info(f"画像 {i+1}/{len(markers)} を生成中: {description[:60]}...")
        try:
            response = client.models.generate_content(
                model=IMAGE_MODEL,
                contents=prompt,
            ...
            if not _save_image_from_response(response, output_path):
                logger.warning(f"スライド{i}に画像パーツがありません。プレースホルダーを使用します。")
                _make_placeholder_image(description, output_path)
        except Exception as e:
            logger.error(f"スライド{i}の画像生成失敗: {e}。プレースホルダーを使用します。")
            _make_placeholder_image(description, output_path)
