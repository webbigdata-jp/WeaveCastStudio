...
- WeaveCastStudio M4 サーバが起動済み（`http://localhost:8765/overlay` にアクセスできる状態）
...
```bash
cd compe_M4
python breaking_news_server.py [DBパス]
```
...
WeaveCast Media       ← MediaWindow のウィンドウキャプチャ
（その他のソース）
```
...
1. `demo_setup.py --seed-db --run --breaking-delay 15` を実行
