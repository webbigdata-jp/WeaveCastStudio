...
処理フロー:
  1. Gemini + Google Search Tool で各国の公式見解と参照URLを検索（英語で検索）
...
def _fetch_page_with_drissionpage(
    ...
    """
    try:
        logger.info(f"    [DrissionPage] ページを開いています: {url}")
        page.get(url)
        ...
        logger.info(f"    [DrissionPage] スクリーンショット保存: {screenshot_path.name}")
        ...
        logger.warning(f"    [DrissionPage] ページ取得失敗 {url}: {e}")
        return ""
...
def _collect_one_country(
    ...
    """
    logger.info(f"{country} の公式声明を収集中...")

    # --- Step A: Gemini + Google Search でURL付き要約を取得（英語で検索） ---
    try:
        ...
        logger.error(f"  -> {country} のGemini検索失敗: {e}")
        return country, {}

    if "NO_STATEMENT_FOUND" in gemini_text:
        logger.warning(f"  -> {country} の公式声明が見つかりませんでした。スキップします。")
        return country, {}

    # --- Step B: URLを抽出してDrissionPageでページを取得 ---
    urls = _extract_urls_from_text(gemini_text)
    logger.info(f"  -> {len(urls)}件のURLを検出: {urls[:3]}")  # 最初の3件をログ表示
    ...
    if page_texts:
        ...
        try:
            ...
            logger.warning(f"  -> {country} の再要約失敗。Gemini検索結果をそのまま使用します: {e}")
    ...
    logger.info(f"  -> 収集完了（{len(final_text)}文字、スクリーンショット{len(screenshot_paths)}件）")
...
def collect_government_statements(
    ...
    logger.info(
        f"情報収集完了: {len(raw_statements)}/{len(topic['countries_of_interest'])}カ国"
    )
