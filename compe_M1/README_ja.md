...
プロジェクトルートの `.env.sample` をコピーして `.env` を作成し、値を設定する（全モジュール共通）。

```bash
cp .env.sample .env
# .env を編集して GOOGLE_API_KEY と LANGUAGE を設定する
```

```ini
GOOGLE_API_KEY=your_api_key_here
LANGUAGE=ja   # BCP-47 言語コード（例: ja / en / ko / zh）
```

`LANGUAGE` は出力言語を制御する BCP-47 コード。  
`shared/language_utils.py` が Live API 用のコード（例: `ja`）と、  
プロンプト埋め込み用の言語名（例: `Japanese`）に自動変換する。  
サポートコード一覧: https://ai.google.dev/gemini-api/docs/live-api/capabilities#supported-languages
...
topics:
  - title_en: "Topic English Name"          # Gemini 検索クエリ・内部キーに使用
    title_target_lang: "トピック表示名"       # 画面表示・ナレーション・画像キャプションに使用
    query_keywords:                          # LANGUAGE=ja の場合は日本語表記
      - "search keyword 1"
      - "search keyword 2"
...
| フィールド | 用途 |
|-----------|------|
| `title_en` | Gemini 検索クエリ生成・`raw_statements` の辞書キー・内部処理に使用 |
| `title_target_lang` | 画面表示・ナレーション原稿・画像キャプション（`LANGUAGE` で設定した言語で記述） |
...
```
WeaveCastStudio/
├── .env                           # GOOGLE_API_KEY + LANGUAGE（全モジュール共通）
...
│   ├── language_utils.py          #   BCP-47 言語設定ローダー（.env の LANGUAGE を読み込み）
