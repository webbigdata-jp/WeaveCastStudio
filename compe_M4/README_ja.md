...
│  │  │（バックグラウンド│   │    │  trumpstruth.org を5分毎に    │  │
│  │  │  スレッド）     │   │    │  ポーリング → Gemini 判定     │  │
│  │  └────────────────┘   │    │  → score≥7.5 で mark_breaking │  │
│  └──────────────────────┘    └───────────────────────────────┘  │
└─────────────────────────────────────────────────────────────────┘
```
...
6. **TrumpMonitor**（バックグラウンドスレッド）が trumpstruth.org を5分毎に監視し、新着投稿を Gemini で判定して ArticleStore に速報として自動投入
...
├── test_trump_monitor.py      # TrumpMonitor 動作確認テストスクリプト
├── trump_monitor_state.json   # TrumpMonitor 最終取得 status ID（自動生成）
├── monitor/                   # バックグラウンドニュース監視モジュール
│   ├── __init__.py
│   └── trump_monitor.py       # Truth Social ポーリング + Gemini 判定
├── overlay/
...
6. **TrumpMonitor** をバックグラウンドスレッドで起動（trumpstruth.org を5分毎に監視）
7. Gemini Live API に接続し、PTT 待機状態に入る
...
[trumpstruth.org](https://trumpstruth.org) を5分毎にポーリングし、トランプ大統領の新着投稿を自動的に ArticleStore に投入するバックグラウンドスレッドです。

**主な機能:**

- **スクレイピング**: `requests` + `BeautifulSoup` でトップページを取得。本文のない ReTruth は自動スキップ。
- **重複排除**: 処理済みの最大 `status_id` を `trump_monitor_state.json` に保存し、次回以降は新しい ID の投稿のみ処理。
- **Gemini 判定**: 新着投稿ごとに `gemini-2.5-flash` に問い合わせ、`importance_score`（0〜10）・`is_breaking` フラグ・一文要約（設定言語）・英語トピックを取得。
- **フィルタリング**: `importance_score < 3.0` の投稿は保存しない。`score >= 7.5` の投稿は `mark_breaking(True)` でティッカーに赤い BREAKING として表示。
- **言語対応**: 要約の言語は `.env` の `LANGUAGE` 設定に従う。
- **拡張性**: `threading.Thread` サブクラスとして統一インターフェースを持つ設計。Reddit 等の追加ソースは `monitor/` 以下に同様のクラスとして追加可能。

**状態ファイル**: `compe_M4/trump_monitor_state.json`（自動生成。削除すると直近の投稿を再処理）
...
スクレイピングと Gemini 判定を独立して確認できます。M4 本体を起動せずに動作確認したい場合に使用してください。
...
| 定数 | デフォルト値 | 説明 |
|------|-------------|------|
| `_DEFAULT_INTERVAL` | `300` | ポーリング間隔（秒） |
| `_BREAKING_THRESHOLD` | `7.5` | Breaking 判定の最低 importance_score |
| `_JUDGE_MODEL` | `gemini-2.5-flash` | 重要度判定に使う Gemini モデル |
| `_REQUEST_TIMEOUT` | `20` | HTTP リクエストタイムアウト（秒） |
...
- `compe_M4/` で `python test_trump_monitor.py` を実行して、スクレイピングと Gemini 判定を単独で確認してください
- `gemini_live_client.py` 起動後のターミナルログに `[TrumpMonitor]` が表示されているか確認してください
- `trump_monitor_state.json` の `last_status_id` が大きすぎる場合、ファイルを削除すると直近の投稿を再処理します
- `https://trumpstruth.org` へのネットワークアクセスを確認してください
